...
def generate_novel_architecture_ui(self):
            ...
            if hasattr(self, 'generation_controller') and self.generation_controller:
                try:
                    ...
                    params = {
                        ...
                        "llm_config_name": self.architecture_llm_var.get()
                    }
                    ...
                    success = loop.run_until_complete(
                        self.generation_controller.generate_novel_architecture(params)
                    )
                    loop.close()
                    ...
                    if success:
                        ...
                        self.safe_log("❌ 小说架构生成失败")
                        
                except Exception as e:
                    self.safe_log(f"❌ 使用控制器生成架构失败: {e}")
                    # 回退到原有逻辑
                    self._generate_architecture_fallback(filepath)
            else:
                # 原有逻辑作为备用
                self._generate_architecture_fallback(filepath)
    ...
    def _generate_architecture_fallback(self, filepath):
        """原有架构生成逻辑作为备用"""
        ...
        try:
            ...
            if hasattr(self, 'generation_controller') and self.generation_controller:
                try:
                    params = {
                        "filepath": filepath,
                        "llm_config_name": self.chapter_outline_llm_var.get()
                    }
                    ...
                    asyncio.set_event_loop(loop)
                    success = loop.run_until_complete(
                        self.generation_controller.generate_chapter_blueprint(params)
                    )
                    loop.close()
                    
                    if success:
                        self.safe_log("✅ 章节大纲生成完成。")
                    else:
                        self.safe_log("❌ 章节大纲生成失败")
                        
                except Exception as e:
                    self.safe_log(f"❌ 使用控制器生成大纲失败: {e}")
                    self._generate_blueprint_fallback(filepath)
            else:
                self._generate_blueprint_fallback(filepath)
                
        except Exception:
            self.handle_exception("生成章节大纲时出错")
        finally:
            self.enable_button_safe(self.btn_generate_directory)

    def _generate_blueprint_fallback(self, filepath):
        """原有大纲生成逻辑作为备用"""
...
def generate_chapter_draft_ui(self):
            ...
            if hasattr(self, 'generation_controller') and self.generation_controller:
                try:
                    params = {
                        ...
                        "llm_config_name": self.prompt_draft_llm_var.get(),
                        "embedding_config": {
                    ...
                    asyncio.set_event_loop(loop)
                    success = loop.run_until_complete(
                        self.generation_controller.generate_chapter_draft(params)
                    )
                    loop.close()
                    ...
                    if success:
                        ...
                        # 显示生成的内容
                        chapter_file = f"{filepath}/第{params['chapter_num']}章_草稿.txt"
                        if os.path.exists(chapter_file):
                            draft_text = read_file(chapter_file)
                            self.master.after(0, lambda: self.show_chapter_in_textbox(draft_text))
                    else:
                        self.safe_log("❌ 章节草稿生成失败")
                        
                except Exception as e:
                    self.safe_log(f"❌ 使用控制器生成草稿失败: {e}")
                    self._generate_draft_fallback()
            else:
                self._generate_draft_fallback()
    ...
    def _generate_draft_fallback(self):
        """原有草稿生成逻辑作为备用"""
        # 原有的复杂逻辑保持不变，包括提示词编辑对话框等
...
def finalize_chapter_ui(self):
            ...
            if hasattr(self, 'generation_controller') and self.generation_controller:
                try:
                    params = {
                        ...
                        "chapter_num": self.safe_get_int(self.chapter_num_var, 1),
                        "word_number": self.safe_get_int(self.word_number_var, 3000),
                        "llm_config_name": self.final_chapter_llm_var.get(),
                        "embedding_config": {
                            "api_key": self.embedding_api_key_var.get().strip(),
                            "base_url": self.embedding_url_var.get().strip(),
                            "model_name": self.embedding_model_name_var.get().strip(),
                    ...
                    asyncio.set_event_loop(loop)
                    success = loop.run_until_complete(
                        self.generation_controller.finalize_chapter(params)
                    )
                    loop.close()
                    ...
                    if success:
                        ...
                        # 显示定稿内容
                        chapter_file = f"{filepath}/第{chap_num}章.txt"
                        if os.path.exists(chapter_file):
                            final_text = read_file(chapter_file)
                            self.master.after(0, lambda: self.show_chapter_in_textbox(final_text))
                    else:
                        self.safe_log("❌ 章节定稿失败")
                        
                except Exception as e:
                    self.safe_log(f"❌ 使用控制器定稿失败: {e}")
                    self._finalize_chapter_fallback()
            else:
                self._finalize_chapter_fallback()
    ...
    def _finalize_chapter_fallback(self):
        """原有定稿逻辑作为备用"""
