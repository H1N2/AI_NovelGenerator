...
def build_character_tab(self):
    self.character_tab = self.tabview.add(t("tabs.character_state"))
    ...
    def update_word_count(event=None):
        ...
        self.character_wordcount_label.configure(text=f"{t('labels.word_count')}：{text_length}")
...
def load_character_state(self):
    ...
    if not filepath:
        messagebox.showwarning(t("messages.warning"), t("messages.set_filepath_first"))
        return
    ...
    self.log(t("messages.character_state_loaded"))
...
def save_character_state(self):
    ...
    self.log(t("messages.character_state_saved"))
